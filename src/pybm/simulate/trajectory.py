...
from pybm.model import Choose, Context, InducedModel
...
def simulate(model: InducedModel, t_eval, context : Context, **kwargs):
    """
    Simulate the model using the given context and time evaluation points.

    Parameters:
    - model: The induced (indexed) model to simulate - the output of `Model.induce()`.
    ...
    - sol: The solution object returned by the ODE solver. `sol.y` is indexed exactly like before -
      ...
      A variable with neither an ODE nor an algebraic equation is held "frozen" at its initial value
      for the whole trajectory instead of raising - a real ProBMoT model can legitimately declare a
      variable (with an initial value) that no instantiated process ever writes an equation for,
      e.g. because the chosen structural variant doesn't need it.
    """
    ...
    def f(t, y):
        ...
        for var in frozen_vars:
            var_ctx[var.index_in_ctx] = initial_values_full[var.index_in_ctx]
    ...
    # reconstruct the full (state + algebraic + frozen) trajectory at every output time point, in
    # the model's original index_in_ctx layout, so callers can keep indexing sol.y by any endo var
